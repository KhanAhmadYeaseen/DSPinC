...
void design_highpass_blackman(double *h, long long int taps, double fc, double fs){
    design_lowpass_blackman(h, taps, fc, fs) ;
...
void design_bandstop_blackman(double *h, long long int taps, double low_cutoff_freq, double high_cutoff_freq, double fs){
    double *h_lp = malloc(taps * sizeof(double));
    double *h_hp = malloc(taps * sizeof(double));

    // Low-pass with lower cutoff
    design_lowpass_blackman(h_lp, taps, low_cutoff_freq, fs);

    // High-pass with upper cutoff
    design_highpass_blackman(h_hp, taps, high_cutoff_freq, fs);

    // Band-stop = LP(fc_low) + HP(fc_high)
    for (long long int i = 0; i < taps; i++){
        h[i] = h_lp[i] + h_hp[i];
    }
...
int main(){
    ...
    design_bandstop_blackman(h, TAPS, low_cutoff_freq, high_cutoff_freq, FS) ;
    write_signal_into_file("filter_bandstop.dat", h, TAPS) ;
    ...
    write_signal_into_file("output_bandstop.dat", output, SIG_LENGTH) ;
    ...
    write_signal_into_file("mags_af_bandstop.dat", mags_af, BINS_LENGTH) ;
    write_signal_into_file("phs_af_bandstop.dat", phs_af, BINS_LENGTH) ; 
